...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_project_root.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggProjectRoot = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_project_root.dart',
      );

      expect(
        ggProjectRoot,
        isNotNull,
        reason: 'Item for executing bin/gg_project_root.dart not found',
      );

      expect(ggProjectRoot['name'], 'gg_project_root.dart');
      expect(ggProjectRoot['type'], 'dart');
      expect(ggProjectRoot['request'], 'launch');
